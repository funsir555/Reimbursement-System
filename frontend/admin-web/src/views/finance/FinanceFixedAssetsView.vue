<template>
  <div class="fa-page">
    ...
    <el-dialog v-model="cardDialogVisible" :title="editingCardId ? '编辑资产卡片' : '新增资产卡片'" width="860px">
      <div class="form-grid">
        <label><span>资产编码</span><el-input v-model="cardForm.assetCode" :disabled="Boolean(editingCardId)" /></label>
        <label><span>资产名称</span><el-input v-model="cardForm.assetName" /></label>
    ...
    <el-dialog v-model="categoryDialogVisible" :title="editingCategoryId ? '编辑资产类别' : '新增资产类别'" width="860px">
      <div class="form-grid">
        <label><span>类别编码</span><el-input v-model="categoryForm.categoryCode" :disabled="Boolean(editingCategoryId)" /></label>
        <label><span>类别名称</span><el-input v-model="categoryForm.categoryName" /></label>
    ...
    <el-dialog v-model="changeDialogVisible" title="新增资产变动单" width="720px">
      <div class="form-grid">
        ...
        <label><span>资产编码</span><el-input v-model="changeLine.assetCode" /></label>
        <label><span>资产名称</span><el-input v-model="changeLine.assetName" /></label>
    ...
    <el-dialog v-model="disposalDialogVisible" title="新增资产处置单" width="520px">
      <div class="form-grid one-col">
        ...
        <label><span>资产编码</span><el-input v-model="disposalLine.assetCode" /></label>
      </div>
...
function normalizeCardPayload(): FixedAssetCardPayload { return { ...cardForm, companyId: filters.companyId, bookCode: filters.bookCode, useCompanyId: filters.companyId, assetCode: trimText(cardForm.assetCode), assetName: trimText(cardForm.assetName), originalAmount: normalizeRequiredMoney(cardForm.originalAmount), accumDeprAmount: normalizeRequiredMoney(cardForm.accumDeprAmount), salvageAmount: normalizeRequiredMoney(cardForm.salvageAmount), usefulLifeMonths: Number(cardForm.usefulLifeMonths || 0), depreciatedMonths: Number(cardForm.depreciatedMonths || 0), remainingMonths: Number(cardForm.remainingMonths || 0), status: cardForm.status || 'IN_USE', canDepreciate: cardForm.canDepreciate ?? true } }
function normalizeCategoryPayload(): FixedAssetCategoryPayload { return { ...categoryForm, companyId: filters.companyId, bookCode: filters.bookCode, categoryCode: trimText(categoryForm.categoryCode), categoryName: trimText(categoryForm.categoryName), usefulLifeMonths: Number(categoryForm.usefulLifeMonths || 0), residualRate: Number(categoryForm.residualRate || 0), depreciable: categoryForm.depreciable ?? true, status: categoryForm.status || 'ACTIVE' } }
function normalizeChangePayload(): FixedAssetChangeBillPayload { return { ...changeForm, ...buildPeriodContext(), billDate: changeForm.billDate || todayText(), lines: changeForm.lines.map((line) => ({ ...line, assetCode: trimText(line.assetCode), assetName: trimText(line.assetName), changeAmount: normalizeOptionalMoney(line.changeAmount), newValue: normalizeOptionalMoney(line.newValue), newSalvageAmount: normalizeOptionalMoney(line.newSalvageAmount) })) } }
function normalizeDisposalPayload(): FixedAssetDisposalBillPayload { return { ...disposalForm, ...buildPeriodContext(), billDate: disposalForm.billDate || todayText(), lines: disposalForm.lines.map((line) => ({ ...line, assetCode: trimText(line.assetCode) })) } }
